...
如果只讲"我做了个 RAG + Agent"，这个项目和市面上的教程没有区别。真正的差异在这四点，
每一点都有**可复现的实测数据**支撑：
...
**能展开的点**：为什么不用 LangGraph（编排只有 200 行，自己写可控可调试）；
步数预算怎么定；工具调挂了怎么处理（错误回灌让 Planner 换策略，而不是 500）。
...
**能展开的点**：为什么评测走 CLI 而不是 HTTP；指标口径怎么定
（Recall 只能在有金标来源的用例上算，否则会被拒答用例稀释——这是实际踩过的坑）。
